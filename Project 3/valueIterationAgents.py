...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for t_state, t_prob in self.mdp.getTransitionStatesAndProbs(state, action):
            ...
            next_q = self.getValue(t_state)
            q += t_prob * (reward + discount * next_q)
        return q
            ...
            if prio_q.isEmpty():
                ...
                for p in predecessors:
                    ...
                    for action in self.mdp.getPossibleActions(p):
                        values[action] = self.getQValue(p, action)
                    policy = values[values.argMax()]
                    diff = abs(self.values[p] - policy)
                    if diff > self.theta:
                        prio_q.update(p, -diff)